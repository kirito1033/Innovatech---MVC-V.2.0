...
const model = 'EstadoUsuarioModel';
...
async function delete_(id) {
  method = 'GET';
  url = URI_ESTADOUSUARIO + LIST_CRUD[3] + '/' + id;
  data = "";
...
async function getDataId(id) {
  method = 'GET';
  url = URI_ESTADOUSUARIO + LIST_CRUD[1] + '/' + id;
...
mainApp.getForm().addEventListener('submit', async function (event) {
  ...
  if (mainApp.setValidateForm()) {
    ...
    if (insertUpdate) {
      method = 'POST';
      url = URI_ESTADOUSUARIO + LIST_CRUD[0];
      data = mainApp.getDataFormJson();
      resultFetch = getData(data, method, url);
      resultFetch.then(response => response.json())
      .then(data => {
      ...
      method = 'POST';
      url = URI_ESTADOUSUARIO + LIST_CRUD[2];
      data = mainApp.getDataFormJson();
